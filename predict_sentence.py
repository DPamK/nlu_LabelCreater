...
class Model(object):
    def __init__(self, parser):
        self.pred_config = parser
        self.args = get_args(self.pred_config)
        self.device = get_device(self.pred_config)
        self.model = load_model(self.pred_config, self.args, self.device)
    ...
    def predict(self, input):
        ...
        words = input.split()
        lines = [words]
        dataset = convert_input_file_to_tensor_dataset(lines, self.pred_config, self.args, self.tokenizer, pad_token_label_id)
        ...
        for batch in tqdm(data_loader, desc="Predicting"):
            ...
            with torch.no_grad():
                ...
                for words, slot_preds, intent_pred in zip(lines, slot_preds_list, intent_preds):
                    output['intent'] = self.intent_label_lst[intent_pred]
                    slots = []
...
if __name__ == "__main__":
    ...
    model = Model(args)
    input = "哦 我 现在 我 我 能 申请 飞 那个 marvo 吗 国航 1 6 1 5"
